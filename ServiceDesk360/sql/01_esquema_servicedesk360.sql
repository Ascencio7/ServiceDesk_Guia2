CREATE DATABASE IF NOT EXISTS servicedesk360_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
CREATE USER IF NOT EXISTS 'servicedesk_app'@'localhost' IDENTIFIED BY 'CAMBIAR_LOCALMENTE';
GRANT SELECT, INSERT, UPDATE, DELETE ON servicedesk360_db.* TO 'servicedesk_app'@'localhost';
FLUSH PRIVILEGES;
USE servicedesk360_db;

CREATE TABLE IF NOT EXISTS usuarios (id_usuario BIGINT PRIMARY KEY AUTO_INCREMENT, nombre VARCHAR(120) NOT NULL, correo VARCHAR(160) NOT NULL UNIQUE, hash_clave VARCHAR(200) NOT NULL, rol VARCHAR(20) NOT NULL, activo BOOLEAN NOT NULL DEFAULT TRUE, CONSTRAINT chk_usuario_rol CHECK (rol IN ('SOLICITANTE','TECNICO','ADMINISTRADOR')));

CREATE TABLE IF NOT EXISTS clientes (id_cliente BIGINT PRIMARY KEY AUTO_INCREMENT, nombre VARCHAR(120) NOT NULL, correo VARCHAR(160) NOT NULL UNIQUE, activo BOOLEAN NOT NULL DEFAULT TRUE);
CREATE TABLE IF NOT EXISTS equipos (id_equipo BIGINT PRIMARY KEY AUTO_INCREMENT, id_cliente BIGINT NOT NULL, codigo_inventario VARCHAR(40) NOT NULL UNIQUE, tipo VARCHAR(80) NOT NULL, marca VARCHAR(80), modelo VARCHAR(80), activo BOOLEAN NOT NULL DEFAULT TRUE, CONSTRAINT fk_equipo_cliente FOREIGN KEY (id_cliente) REFERENCES clientes(id_cliente));
CREATE TABLE IF NOT EXISTS tecnicos (id_tecnico BIGINT PRIMARY KEY AUTO_INCREMENT, nombre VARCHAR(120) NOT NULL, correo VARCHAR(160) NOT NULL UNIQUE, especialidad VARCHAR(100), activo BOOLEAN NOT NULL DEFAULT TRUE);
